...
class TVTuner:
    ...
    def scan_channels(self):
        ...
        # Use -o 1 to output in czap/tzap format (compatible with czap tuner)
        cmd = f"w_scan -A 1 -ft -c US -o 1"
        ...
        self.scan_proc = subprocess.Popen(
            cmd.split(),
            stdout=zap_file,  # Save to channels.zap for czap
    ...
    def tune_channel(self, channel):
        ...
        # We need a channels.zap file for czap tuner.
        # The file should be in czap format generated by w_scan -o 1.
        # Each line contains: STATION_NAME:frequency:params...
        zap_path = os.path.join(CONFIG_DIR, "channels.zap")
        ...
        if not os.path.exists(zap_path):
            ...
            # Generate it in czap format (-o 1)
            os.makedirs(CONFIG_DIR, exist_ok=True)
        ...
        logger.info(f"[TUNE] Running czap command: {' '.join(cmd)}")
        ...
        if self.zap_proc.poll() is not None:
            ...
            logger.error(f"[TUNE] czap exited immediately with code {self.zap_proc.returncode}")
    ...
    def _find_zap_entry_by_name(self, zap_path, station_name):
        """
        channels.zap lines in czap format look like:
          WJBK:177000000:INVERSION_AUTO:AUTO:AUTO:AUTO
        Extract the station name before the colon and match against station_name
        """
        try:
