...
sys.path.insert(0, '/opt/homerun-clone')
...
class TVTuner:
    def __init__(self):
        ...
        self.channels = {}
        self.scanner = ChannelScanner(self.device_path)
        self.load_channels()
    ...
    def load_channels(self):
        ...
        try:
            ...
            self.channels = {
                "2.1": {"name": "CBS", "frequency": "177000000", "signal_strength": 0},
                "4.1": {"name": "NBC", "frequency": "193000000", "signal_strength": 0},
                "7.1": {"name": "ABC", "frequency": "175000000", "signal_strength": 0},
                "13.1": {"name": "PBS", "frequency": "213000000", "signal_strength": 0}
            }
            self.save_channels()
    ...
    def scan_channels(self, signal_threshold=20, background=True):
        """Scan for available channels using enhanced scanner"""
        try:
            if background:
                # Start background scan
                success = self.scanner.scan_in_background(signal_threshold)
                if success:
                    # Return immediately, scan status will be updated via WebSocket
                    return {"success": True, "status": "started", "background": True}
                else:
                    return {"success": False, "error": "Scan already in progress"}
            else:
                # Synchronous scan
                success = self.scanner.perform_full_scan(signal_threshold)
                if success:
                    self.load_channels()  # Reload channels after scan
                return {"success": success, "channels": len(self.channels)}
    ...
    def get_scan_status(self):
        ...
        return self.scanner.get_scan_status()
    
    def quick_scan(self):
        """Perform quick scan of popular channels"""
        try:
            success = self.scanner.quick_scan()
            if success:
                self.load_channels()
            return {"success": success, "channels": len(self.channels)}
        except Exception as e:
...
def scan_channels():
    ...
    signal_threshold = data.get('signal_threshold', 20)
    background = data.get('background', True)
    
    result = tuner.scan_channels(signal_threshold, background)
    return jsonify(result)

@app.route('/api/quick-scan', methods=['POST'])
def quick_scan():
    result = tuner.quick_scan()
    return jsonify(result)

@app.route('/api/scan/status')
def scan_status():
    status = tuner.get_scan_status()
...
def handle_connect():
    ...
    emit('status', {
        ...
        "scan_status": tuner.get_scan_status()
    })

def background_scan_monitor():
    """Background task to monitor scan progress and update clients"""
    while True:
        try:
            scan_status = tuner.get_scan_status()
            if scan_status['status'] == 'running':
                socketio.emit('scan_progress', scan_status)
            elif scan_status['status'] == 'complete':
                # Reload channels and notify clients
                tuner.load_channels()
                socketio.emit('scan_complete', {
                    'channels': tuner.channels,
                    'channels_found': len(tuner.channels)
                })
...
monitor_thread = threading.Thread(target=background_scan_monitor)
            ...
            <div class="col-md-3">
                <div class="card">
                    <div class="card-header">
                        <h5>Channels <span id="channelCount" class="badge bg-primary ms-2">{{ channels|length }}</span></h5>
                        <div class="btn-group-vertical w-100 mt-2">
                            <button class="btn btn-sm btn-outline-primary" onclick="showScanModal()">Full Scan</button>
                            <button class="btn btn-sm btn-outline-secondary" onclick="quickScan()">Quick Scan</button>
                        </div>
                    </div>
                    <div class="card-body">
                        <div id="channelList" class="list-group">
                            ...
                            <button class="list-group-item list-group-item-action d-flex justify-content-between align-items-center" 
                                ...
                                {% if info.signal_strength is defined %}
                                <span class="badge bg-{% if info.signal_strength > 70 %}success{% elif info.signal_strength > 40 %}warning{% else %}danger{% endif %}">
                ...
                <div class="card mt-3">
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                <small class="text-muted">Scan Status: <span id="scanStatus">Idle</span></small>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Channel Scanner</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <div class="mb-3">
                        <label class="form-label">Signal Threshold:</label>
                        <input type="range" class="form-range" id="signalThreshold" min="10" max="90" value="20">
                    ...
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" id="identifyNames" checked>
                        <label class="form-check-label" for="identifyNames">
                            Attempt to identify channel names (slower)
                        </label>
                    </div>
                    ...
                    <div id="scanProgress" class="mt-3" style="display: none;">
                        ...
                        <div id="scanResults" class="mt-2">
                            <small class="text-muted">Preparing scan...</small>
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                    <button type="button" class="btn btn-primary" id="startScanBtn" onclick="startFullScan()">
                        Start Scan
                    </button>
                    <button type="button" class="btn btn-success" id="finishScanBtn" onclick="finishScan()" 
                            style="display: none;">
                        Finish
                    </button>
                </div>
...
'''
        ...
        scanner_script = f'''#!/bin/bash
echo "HomeRun Clone - Standalone Channel Scanner"
echo "========================================="

# Copy channel scanner to project directory
cp /home/claude/channel_scanner.py {self.project_root}/
chmod +x {self.project_root}/channel_scanner.py
...
echo "  Channel file: {self.project_root}/config/channels.json"
echo "  Scan results: {self.project_root}/config/scan_results.json"
echo ""
echo "Start the web interface to view channels:"
echo "  {self.project_root}/scripts/service.sh start"
'''
        
        with open(f"{self.project_root}/scripts/scan_channels.sh", 'w') as f:
            f.write(scanner_script)
        os.chmod(f"{self.project_root}/scripts/scan_channels.sh", 0o755)
...
case "$1" in
    ...
    scan)
        ...
        {self.project_root}/scripts/scan_channels.sh
        ;;
    *)
...
'''
    ...
    def run_setup(self):
        ...
        try:
            ...
            # Copy enhanced scanner to project
            scanner_src = "/home/claude/channel_scanner.py"
            scanner_dst = f"{self.project_root}/channel_scanner.py"
            if Path(scanner_src).exists():
                self.run_command(f"cp {scanner_src} {scanner_dst}", "Copying enhanced channel scanner")
                os.chmod(scanner_dst, 0o755)
            ...
            print(f"2. Run channel scan: {self.project_root}/scripts/service.sh scan")
            print(f"3. Start the service: {self.project_root}/scripts/service.sh start")
            print(f"4. Enable auto-start: {self.project_root}/scripts/service.sh enable")
            print(f"5. Access web interface: http://192.168.1.171:{self.web_port}")
            print(f"6. Check logs: {self.project_root}/scripts/service.sh logs")
            print("\nChannel Scanner Features:")
            print("- Full frequency scan with signal strength detection")
            print("- Automatic subchannel detection")
            print("- Channel name identification (when available)")
            print("- Real-time progress updates via web interface")
            print("- Quick scan for popular channels")
            print("\nWeb Interface Features:")
            print("- Interactive channel scanner with progress bar")
            print("- Signal strength indicators for each channel")
            print("- Real-time scan progress via WebSocket")
            print("- Adjustable signal threshold")
            print("\nFor Jellyfin integration:")
            print(f"- Configure Jellyfin to use: http://192.168.1.171:{self.streaming_port}")
            print(f"- Run: python3 jellyfin_integration.py")
